...
public class javaxEmailReceiver {
	 ...
	 * * @param protocol either "imap" or "pop3" * @param host * @param port
	...
	public void downloadEmails(String protocol, String host, String port, String userName, String password) {
		//메일 서버 정보를 가지고있는 Properties.
		Properties properties = getServerProperties(protocol, host, port);
		
		//메일 서버와 연결된 Session을 얻어온다.
		Session session = Session.getDefaultInstance(properties);

		try { 
			// connects to the message store
			Store store = session.getStore(protocol);
			System.out.println(store);  //  imap://User@imap.gmail.com	
			store.connect(userName, password);
			System.out.println(store); // imap://slsldkf1234%40gmail.com@imap.gmail.com
			// opens the inbox folder
			Folder folderInbox = store.getFolder("INBOX");
			System.out.println(folderInbox); // INBOX
			folderInbox.open(Folder.READ_ONLY);
			System.out.println(folderInbox); // INBOX

			// fetches new messages from server
			Message[] messages = folderInbox.getMessages();
			for (int i = 0; i < 30; i++) { // messages.length
	...
	/**
	 * * Returns a list of addresses in String format separated by comma * * @param
	 * address an array of Address objects * @return a string represents a list of
	 * addresses
	 */
	...
	public static void main(String[] args) { 
		...
		// 메일을 가져올수 있는 프로토콜 IMAP, POP3.
		// for IMAP, IMAP을 지원하는 메일서버의 정보 설정.
